...
function bayarBarang() {
	...
	total *= hari;
	listLogBarang.innerHTML += `<h3>Total:<small class="text-muted"> ${formatRupiah(total.toString(), 'Rp.')}</small></h3>`;

	daftarbeliBarang = [];
	showBeliBarang();
	alert(`Total Sewa  : ${formatRupiah(total.toString(), 'Rp.')}`);
	alert('Terima kasih sudah menyewa');
}
